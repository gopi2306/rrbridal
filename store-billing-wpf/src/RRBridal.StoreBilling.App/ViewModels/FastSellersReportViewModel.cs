...
public partial class FastSellersReportViewModel : ObservableObject
{
    ...
    public bool HasRows => SellerRows.Count > 0;
    public string TruncatedMessage { get; private set; } = "";

    public FastSellersReportViewModel(AppServices services)
    {
        _services = services;
    ...
    private async Task Search()
    {
        ...
        IsBusy = true;
        HasError = false;
        StatusMessage = "Loading fast sellers report…";
        try
        {
            var response = await _services.SkuSalesReports.LoadFastSellersAsync(query);
            SellerRows.Clear();
            ...
            TruncatedMessage = response.Truncated
                ? $"Showing the top {response.Limit:N0} of {response.Total:N0} SKUs. Narrow the filters for a complete ranking."
                : "";
            ...
            StatusMessage = IsEmpty
                ? $"No posted SKU sales found from {DateFrom:dd MMM yyyy} to {DateTo:dd MMM yyyy} · {filter}."
                : $"{response.Totals.SkuCount:N0} SKU(s) · {filter} · updated {DateTime.Now.ToString("T", InCulture)}";
        }
        catch (Exception ex)
        {
            SellerRows.Clear();
            ...
            HasError = true;
            OnPropertyChanged(nameof(HasRows));
            StatusMessage = "Could not load fast sellers report: " + ex.Message;
        }
        finally
        {
            IsBusy = false;
    ...
    private async Task DownloadExcel()
    {
        ...
        {
            Title = "Save fast sellers report",
            Filter = "Excel workbook (*.xlsx)|*.xlsx",
            FileName = $"fast-sellers-{query.From:yyyy-MM-dd}-to-{query.To:yyyy-MM-dd}{counterSuffix}.xlsx",
            AddExtension = true,
            DefaultExt = ".xlsx",
        };
        ...
        try
        {
            await _services.SkuSalesReports.SaveFastSellersExportAsync(dialog.FileName, query);
    ...
    private bool TryBuildQuery(out SkuSalesReportQuery query)
    {
        ...
        query = new SkuSalesReportQuery
        {
            ...
            Limit = SkuSalesReportService.FastSellersDefaultLimit,
        };
        return true;
    }
