...
See [sync-protocol.md](sync-protocol.md) for payload shapes from store billing WPF.
...
| `itemsSold` | Invoice line qty minus return line qty |
...
Dates use **UTC** (`payload.createdAtUtc` on bills/returns; document `createdAt` as fallback).
