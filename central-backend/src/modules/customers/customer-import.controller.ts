...
export class CustomerImportController {
  ...
  downloadExcelTemplate(@Res() res: Response): void {
    const buffer = this.customerImportService.buildExcelTemplate();
  ...
  async importExcel(
    ...
    return await this.customerImportService.runFromExcelBuffer(file.buffer, {
  ...
  async importFile(
    ...
    return await this.customerImportService.runFromBuffer(file.buffer, file.originalname ?? 'upload.xlsx', {
      dryRun: dryRun === 'true' || dryRun === '1',
    });
  }
}
